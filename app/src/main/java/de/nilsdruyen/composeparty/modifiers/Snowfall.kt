...
internal fun Modifier.snowfall(
    ...
    onSizeChanged { size -> fluffyState = fluffyState.resize(size, drawables) }
        .clipToBounds()
        .drawWithContent {
            drawContent()
...
private data class SnowfallState(
    ...
    fun resize(size: IntSize, drawables: List<Drawable>): SnowfallState = copy(
...
//        canvas.drawCircle(center = position, radius = height / 3f, paint = snowflakePaint)
    }
