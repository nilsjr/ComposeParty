...
private const val PawAnimationDuration = 1200

private fun calculate(from: Float, to: Float, progress: Float): Float {
    val diff = (to - from)
    val p = ((progress - from) / diff).coerceIn(0f, 1f)
    Timber.d("progress: $progress - $diff")
    return p
}
...
fun PawIcon(
    ...
    val paw01 = calculate(0f, .25f, progress)
    val paw02 = calculate(.2f, .5f, progress)
    val paw03 = calculate(.4f, .7f, progress)
    val paw04 = calculate(.7f, .9f, progress)
