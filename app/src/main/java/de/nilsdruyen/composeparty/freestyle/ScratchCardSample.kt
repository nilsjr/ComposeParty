    ...
    Box(
        modifier = Modifier
            ...
            .onGloballyPositioned {
                ...
                if (imageSize == Size.Zero) imageSize = it.size.toSize()
            },
        ...
        AsyncImage(
            ...
            onState = {
                if (it is AsyncImagePainter.State.Success) {
                    Timber.d("size " + it.painter.intrinsicSize)
        ...
        AnimatedVisibility(
            ...
            Overlay(imageSize) { showCanvas = false }
        }
    }
}

@Composable
fun Overlay(imageSize: Size, hideCanvas: () -> Unit) {
...
private fun View.capture(
    ...
    try {
        ...
        val centerW = this.width / 2
        val centerH = this.height / 2
        val left = centerW - (imageSize.width / 2)
        val right = centerW + (imageSize.width / 2)
        val top = centerH - (imageSize.height / 2)
        val bottom = centerH + (imageSize.height / 2)

        val calculatedRectangle =
            Rect(left.roundToInt(), top.roundToInt(), right.roundToInt(), bottom.roundToInt())

        Timber.d("size calc $calculatedRectangle")
